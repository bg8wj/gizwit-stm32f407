...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueTemperture_ALM != cur->valueTemperture_ALM)
    {
        GIZWITS_LOG("valueTemperture_ALM Changed\n");
        ret = 1;
    }
    if(last->valuehum_ALM != cur->valuehum_ALM)
    {
        GIZWITS_LOG("valuehum_ALM Changed\n");
        ret = 1;
    }

    if(last->valueTemperture != cur->valueTemperture)
    {
        if(gizGetTimerCount()-lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueTemperture Changed\n");
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(Temperture_ALM_BYTEOFFSET,Temperture_ALM_BITOFFSET,Temperture_ALM_LEN,(uint8_t *)devStatusPtr,dataPoints->valueTemperture_ALM);
    gizStandardCompressValue(hum_ALM_BYTEOFFSET,hum_ALM_BITOFFSET,hum_ALM_LEN,(uint8_t *)devStatusPtr,dataPoints->valuehum_ALM);
    gizByteOrderExchange((uint8_t *)devStatusPtr->alertBitBuf,sizeof(devStatusPtr->alertBitBuf));

    devStatusPtr->valueTemperture = gizY2X(Temperture_RATIO,  Temperture_ADDITION, dataPoints->valueTemperture); 
